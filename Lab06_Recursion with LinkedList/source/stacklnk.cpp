...
// Creates a stack node containing dataItem elem and next pointer
// nextPtr.
...
// Removes the topmost dataItem from a stack and returns it.

{
	StackNode<SE> *p;   // Pointer to popped node
	SE temp;            // Temporarily stores popped dataItem
...
// Removes all the dataItems from a stack.
...
// Linked list implementation. Outputs the dataItems in a stack. If
...
template <class SE>
Stack<SE>::Stack(const Stack& valueStack)     // Copy constructor
{
	StackNode<SE>* p = valueStack.top;
	StackNode<SE>* q = top;
	while (p != NULL)
	{
		q->next = p->next;
