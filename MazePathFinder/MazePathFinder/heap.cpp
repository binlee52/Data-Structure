...
**/
{
	...
	if (bottom > root) // not empty
	{
		...
		else if (dataItems[parent].pty() == dataItems[bottom].pty())
		{
			if (dataItems[parent].pty2() < dataItems[bottom].pty2())
			{
				Swap(dataItems[parent], dataItems[bottom]);
				Reheapup(root, parent);
			}
	...
	if (leftchild <= bottom)	// left child is part of the heap
	{
		...
		else		// if two child
		{
			if (dataItems[leftchild].pty() < dataItems[rightchild].pty())
				maxchild = rightchild;
			else if ((dataItems[leftchild].pty() == dataItems[rightchild].pty())
				&& (dataItems[leftchild].pty2() < dataItems[rightchild].pty2()))
				maxchild = rightchild;
			else
				maxchild = leftchild;
		}
		...
		else if ((dataItems[root].pty() == dataItems[maxchild].pty())
			&& (dataItems[root].pty2() < dataItems[maxchild].pty2()))
		{
			Swap(dataItems[root], dataItems[maxchild]);
			Reheapdown(maxchild, bottom);
		}
